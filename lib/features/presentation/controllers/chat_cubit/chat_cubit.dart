...
class ChatCubit extends Cubit<ChatState> {
  ...
  Future<void> deleteConversation(String contactId) async {
    try {
      ...
      final currentChatRef = FirebaseFirestore.instance
          .collection('users')
          .doc(currentUserId)
          .collection('chats')
          .doc(contactId);

      final currentChatMessagesRef = currentChatRef.collection('messages');

      // Delete all documents within the 'messages' subcollection
      await currentChatMessagesRef.get().then((snapshot) {
        for (DocumentSnapshot doc in snapshot.docs) {
          doc.reference.delete();
        }
      });

      // Delete the conversation document for the current user
      await currentChatRef.delete();

      // Handle success
      // You can emit a state or perform any necessary actions here
      print('Conversation deleted successfully');
    } catch (e) {
      // Handle error
      // You can emit an error state or perform any necessary actions here
