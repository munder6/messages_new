...
////////////
  ...
  Future<void> getContactsOnMessageMe() async {
    emit(GetContactsOnAppLoadingState());
    //contactOnWhats = {};
    final result = await _getContactsOnWhatsUseCase(const NoParameters());
    result.fold(
      (l) => emit(GetContactsOnAppErrorState()),
      (r) {
        contactOnWhats.addAll(r);
        emit(GetContactsOnAppSuccessState());
      },
    );
  }
